...
* 将完整的 `response_message` 对象（包含 `tool_calls` 字段）添加到消息历史
  * 原生的OpenAI API 的 `MiniMax-M2.1` `MiniMax-M2.1-lightning` `MiniMax-M2` 模型 `content` 字段会包含 `<think>` 标签内容，需要完整保留
...
| 模型名称                   | 说明                               |
| :--------------------- | :------------------------------- |
| MiniMax-M2.1           | 强大多语言编程实力，全面升级编程体验  (输出速度约60tps) |
| MiniMax-M2.1-lightning | M2.1 极速版：更快，更敏捷  (输出速度约100tps)   |
