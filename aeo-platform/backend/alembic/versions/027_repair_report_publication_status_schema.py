"""repair report publication status schema drift
...
def upgrade() -> None:
    bind = op.get_bind()
    if not _has_column(bind, "brand_report_versions", "publication_status"):
        op.add_column(
...
def downgrade() -> None:
    ...
    if _has_column(bind, "brand_report_versions", "publication_status"):
