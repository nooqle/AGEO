...
def test_generated_panorama_alias_sets_deterministic_route():
    resolution = _resolve_option("continue_panorama")

    assert canonicalize_confirmation_id("continue_panorama") == "panorama"
    assert resolution.user_decisions["a3_mode"] == "baseline_dynamic"
    assert resolution.state_updates["analysis_mode"] == "baseline"
    assert resolution.state_updates["next_required_action"]["tool_name"] == (
        "question_simulation"
...
def test_generated_scenario_alias_routes_to_persona_generation():
    resolution = _resolve_option("scenario_first")

    assert canonicalize_confirmation_id("scenario_first") == "persona_first"
    assert resolution.user_decisions["a3_mode"] == "persona"
    assert resolution.state_updates["next_required_action"]["tool_name"] == (
        "persona_generation"
    )
...
def test_generated_direct_question_alias_routes_to_direct_ask():
    resolution = _resolve_option("ask_questions")

    assert canonicalize_confirmation_id("ask_questions") == "ask"
    assert resolution.user_content == "我想直接提问。"
    assert "next_required_action" not in resolution.state_updates
...
def test_generated_confirmation_labels_are_known():
    ...
    assert resolve_known_confirmation_label("continue_panorama") == "品牌全景分析"
    assert resolve_known_confirmation_label("scenario") == "场景细化分析"
    assert resolve_known_confirmation_label("scenario_first") == "场景细化分析"
    assert resolve_known_confirmation_label("ask_questions") == "直接提问"
