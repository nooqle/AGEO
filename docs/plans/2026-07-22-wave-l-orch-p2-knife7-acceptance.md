...
**不是「拆完大面积调不通」。**  
策略仍是 **搬家 + 再导出**，主入口与绝大多数路径可通。  

**但第七刀曾引入一处真实回归（已修）：**  
`knowledge_fallback._infer_knowledge_fallback_tool` 漏导入 `_resolve_bounded_history_answer_query` → 在「过往资料导出/回答」fallback 路径会 `NameError`。  
影响面：**主 Chat 在 LLM 未选工具时的历史资料 fallback**，不是生产线拓扑/配方/Chat 编排短路主路径。  
处理：补 import + 钉死单测；全量 orch 相关 **73 passed**。
...
| 风险 | 等级 | 缓解 |
|---|---|---|
| 漏 import 导致 `NameError` | **中（本刀已发生 1 次）** | 模块 AST 依赖扫描 + 路径单测；继续强制跑 fallback 钉死 |
