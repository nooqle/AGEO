...
日期：2026-09-23。状态：第五平台代码已开发并通过聚焦单测；开发工作树已配置独立千问 API Key，`qwen3.8-max` 完成一题真实联网搜索试采；登录态浏览器试采尚未验收。工作树：`D:\AGEO-worktrees\qwen-fifth-platform-20260923`，分支 `codex/qwen-fifth-platform-20260923`，基线 `c51be52f3164372dff6eee799a0a83428f4a32f0`（最近一次成功的 demo 部署 Action 所用 SHA；尚未用服务器当前链接独立复核）。`D:\AGEO-main` 的 `origin/main` 仍在 2026-06-25 的 `65ee2c2`，因此本任务从已部署代码建立隔离工作树，不能把旧 main 当成当前运行实现。未改线上配置、未部署。

## 当前实现与未验收范围

已接入公开平台 `qwen`、逐平台 API/浏览器方式、旧拓扑默认关闭千问、Qwen3.8 Max DashScope 多模态流式搜索、`[ref_数字]` 正文引用与检索候选分离、Qwen 专属 AIO context、登录后采集门闸，以及 A4/A5/Control 和画布的第五平台显示。浏览器初始化失败会产生可见失败行，避免报告漏掉计划中的千问；开始运行前会核对服务器权威拓扑与前端开关。独立审查已修复流式 401/429 异常、普通编号误判引用、五平台报告截断、零平台误回退等问题。

开发环境的真实 API 试采已验证认证、搜索来源、正文引用和 token 用量：`qwen3.8-max` 的一次公开问题请求约 3 秒完成，检索来源 10 条、正文匹配引用 1 条、输入/输出 6059/43 token。供应商未返回搜索工具次数，费用仍按未知处理。`qwen.com` 当前跳转至 `qianwen.com`；匿名“日常”页显示 Qwen3.7-千问，“工作”页可选 Qwen3.8-Max。用户已确认 API 用 3.8 Max，浏览器允许网站当前模型。浏览器登录态、搜索来源抓取、AIO/CDP 会话与 Control 投影仍须验收；不能以匿名网页试答代替正式采集。快速连续切换连线时，前端异步 PUT 仍可能发生低频先后顺序竞态；运行前 GET 校验可以阻止常见的单次未保存情形，不能替代服务端运行快照。以下“方案边界”和“验收门槛”仍是上线前要求，不等同于已验收事实。
...
千问网页目前在未登录状态下允许提问。2026-09-23 对公开问题“安利纽崔莱是什么？”做了单次实测：`qwen.com` 跳转到 `qianwen.com`，日常页显示 `Qwen3.7-千问`，自动检索 2 个关键词、参考 9 篇资料，回答正文与来源链接均可在页面读取；输入框为 `[contenteditable="true"][role="textbox"]`，发送按钮有 `aria-label="发送消息"`，正文含 `.qk-markdown-react`。另一匿名试题仅返回正文链接，未展示独立搜索来源，因此不能推断每题都会搜索。**产品要求仍是登录后采集：匿名可回答不等于可用于 Specta 正式采集。** 点击“登录”后可见“用千问 APP 扫码登录”，并有手机号验证码、淘宝及支付宝入口；登录态仍待确认。这只证明当前匿名网页和登录入口的行为，不证明生产 AIO/CDP 会话、登录状态、限流、批量稳定性或 API 与网页模型等价。网页结构属于易变运行时选择器，必须有 DOM 稳定性和失败证据。
...
实现选用 DashScope 多模态生成端点与用户指定的 `qwen3.8-max`，因为可同时要求联网搜索、来源与明确的引用角标。真实请求表明必须使用 `incremental_output=true` 并拼接增量片段；本采集场景显式设置 `enable_thinking=false`，避免默认思考产生长时延或仅有推理而无最终答案。该组合已通过一题真实请求，批量完成率、单题费用和生产额度仍待验收。`qianwen.com` 日常页上的 `Qwen3.7-千问` 不是公开 API 型号的映射证明。
...
本轮密钥配置后的兼容修正按范围 1、架构新颖度 0、可逆性 0、影响风险 1、可观测性 0，计 **2 / R0**：当前 Codex 直接修正增量流解析、默认 `qwen3.8-max` 与网页跳转域名；以官方接口文档、一次 3.8 Max 真实搜索及 65 项聚焦测试作为证据。既有独立复核不覆盖这次新增差异；登录态网页与生产服务仍待独立验收，未触发 Grok 或 Astra 升级。
...
独立代码审查又指出三处数据与可靠性风险：来源候选未穿过 AIO packet/平台投影、旧对话答案可能误归入新题、浏览器关闭可能被误报为无搜索。当前修订已在结果包及投影保留 `retrieved_sources`，要求新对话成功清空旧回答，按最新回答的消息组读取检索状态并从该组打开来源面板，且将页面关闭错误保留给现有重试机制。聚焦测试覆盖这些条件；真实 AIO 会话连续多题仍是上线前门槛。
