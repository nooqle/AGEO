...
const BRAND_EVIDENCE = '31, 122, 107';
...
/** Map normalized value (10-100) to evidence-color opacity tier */
function valueToColor(value: number): string {
  if (value >= 80) return `rgba(${BRAND_EVIDENCE}, 1)`;
  if (value >= 60) return `rgba(${BRAND_EVIDENCE}, 0.85)`;
  if (value >= 40) return `rgba(${BRAND_EVIDENCE}, 0.65)`;
  if (value >= 25) return `rgba(${BRAND_EVIDENCE}, 0.45)`;
  return `rgba(${BRAND_EVIDENCE}, 0.3)`;
}
      ...
      <div className="p-5 bg-[var(--bg-elevated)] border border-[var(--border-subtle)] rounded-xl">
        <div className="flex items-center gap-6">
          <div className="text-center min-w-[100px]">
            <div className="text-4xl font-semibold text-[var(--brand-text)]">
              {total_keywords}
            </div>
          ...
          <div className="flex-1">
            <div className="grid grid-cols-2 gap-3">
              <div className="text-center p-2 rounded-lg" style={{ backgroundColor: 'var(--brand-bg)' }}>
          ...
          <button
            ...
            className={cn(
              ...
              selectedPlatform === null
                ? 'bg-[var(--brand-bg)] text-[var(--brand-text)] border-[var(--brand-border)]'
          ...
          {platforms.map((plat) => (
            <button
              ...
              style={selectedPlatform === plat ? {
                backgroundColor: `${getPlatformColor(plat) || '#1F7A6B'}15`,
                color: getPlatformColor(plat) || 'var(--brand-text)',
                borderColor: `${getPlatformColor(plat) || '#1F7A6B'}30`,
              } : undefined}
            >
              <span
                className="w-2 h-2 rounded-full flex-shrink-0"
                style={{ backgroundColor: getPlatformColor(plat) || 'var(--brand-primary)' }}
              />
          ...
          <div className="space-y-2">
            {rankingKeywords.map((kw, i) => (
              <div key={kw.word}>
                <button
                  onClick={() => setExpandedKeyword(expandedKeyword === kw.word ? null : kw.word)}
                  className="w-full flex items-center gap-3 p-3 bg-[var(--bg-elevated)] border border-[var(--border-subtle)] rounded-lg hover:border-[var(--brand-border)] transition-colors text-left"
                >
                  <span className={cn(
                    'w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0',
                    i < 3 ? 'bg-[var(--brand-bg)] text-[var(--brand-text)]' : 'bg-[var(--bg-tertiary)] text-[var(--text-tertiary)]'
                  )}>
                    {i + 1}
                  </span>
                ...
                {expandedKeyword === kw.word && kw.contexts.length > 0 && (
                  <div className="mt-1 ml-9 mr-3 mb-1">
                      <div className="p-3 rounded-lg border border-[var(--border-subtle)]" style={{ backgroundColor: 'var(--bg-report-muted)' }}>
                        ...
                        {kw.contexts.map((ctx, ci) => (
                          <div key={ci} className="flex items-start gap-2">
                            <span
                              ...
                              style={{ backgroundColor: getPlatformColor(ctx.platform) || 'var(--brand-primary)' }}
                            />
                            <div className="min-w-0">
                              <span className="text-[10px] font-medium" style={{ color: getPlatformColor(ctx.platform) || 'var(--brand-text)' }}>
...
function ExpandedKeywordDetail({
  ...
  return (
    <div className="mt-3 p-4 rounded-lg border border-[var(--brand-border)]" style={{ backgroundColor: 'var(--bg-report-muted)' }}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <span className="text-[15px] font-semibold text-[var(--brand-text)]">{keyword.word}</span>
...
function highlightWord(text: string, word: string): React.ReactNode {
  ...
  while (remaining.length > 0) {
    ...
    parts.push(
      <span key={keyIdx++} className="font-semibold text-[var(--brand-text)]">
        {word}
      </span>
    );
